<template>
  <div class="container">
    ...
    <div class="grid grid-cols-5 gap-4 px-8">
      <div v-for="r in rooms" class="px-4 py-10 bg-gray-200 rounded-lg shadow-md">
        ...
        <div class="flex justify-center">
          ...
          <button class="btn btn-sm btn-warning ml-4" @click="removeRoom(r)">删除</button>
        </div>
      </div>
    </div>
  </div>
...
const removeRoom = async (id: any) => {
  const { data, error } = await useFetch(`${baseURL}/room/${id}`).delete({ domain: 'default' }).json()
  if (error.value == null) {
